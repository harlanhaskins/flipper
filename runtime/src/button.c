...
LF_WEAK int button_configure(void) {
	return lf_invoke(lf_get_current_device(), &_button, _button_configure, fmr_int_t, NULL);
}

LF_WEAK uint8_t button_read(void) {
	return lf_invoke(lf_get_current_device(), &_button, _button_read, fmr_int8_t, NULL);
}
