...
LF_WEAK int uart0_configure(uint8_t baud, uint8_t interrupts) {
	lf_invoke(lf_get_current_device(), &_uart0, _uart0_configure, fmr_int_t, fmr_args(fmr_infer(baud), fmr_infer(interrupts)));
	return lf_success;
}

LF_WEAK int uart0_ready(void) {
	return lf_invoke(lf_get_current_device(), &_uart0, _uart0_ready, fmr_int_t, NULL);
}
